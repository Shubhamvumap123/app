    ...
    <footer className="bg-teal-950 text-white py-16">
      <div className="container mx-auto px-4">
        <div className="grid md:grid-cols-4 gap-8 mb-12">
          <div>
            ...
            <p className="text-teal-100 text-sm mb-6 leading-relaxed">
              Southern Slam Academy is dedicated to developing tennis players of
              all levels in a supportive and professional environment. Join us
          ...
          <div>
            ...
            <ul className="space-y-3">
              {["Home", "About", "Programs", "Facilities", "Coaches"].map(
                (item) => (
                  <li key={item}>
                    <Link
                      href={item === "Home" ? "/" : `/${item.toLowerCase()}`}
                      className="text-teal-100 hover:text-white hover:translate-x-1 transition-all inline-block"
                    >
                      {item}
                    </Link>
                  </li>
          ...
          <div>
            ...
            <form className="flex flex-col gap-2" onSubmit={handleSubscribe}>
              <Input
                ...
                className="bg-teal-900 border-teal-800 text-white placeholder:text-teal-400 focus-visible:ring-teal-500 disabled:opacity-50"
                disabled={loading}
                required
              />
              <Button
                type="submit"
                className="w-full bg-teal-600 hover:bg-teal-700 text-white font-semibold disabled:opacity-75 disabled:cursor-not-allowed"
                disabled={loading}
              >
                {loading ? (
                  <>
        ...
        <div className="pt-8 border-t border-teal-900 flex flex-col md:flex-row justify-between items-center text-sm text-teal-300">
          <p>
            © {new Date().getFullYear()} Southern Slam Academy. All rights
            reserved.
          </p>
